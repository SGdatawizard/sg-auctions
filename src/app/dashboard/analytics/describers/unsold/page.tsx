...
export default function UnsoldLotsPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // Paginated lots fetch — include sold so we can filter in JS
      const allLots: RawLot[] = [];
      {
        ...
        while (true) {
          const { data, error } = await supabase
            .from("lots")
            .select("id, lot_number, stock_number, receipt_no, title, description, estimate_low, estimate_high, reserve, auction_id, department, category, sold")
            .in("auction_id", auctionIds)
            .range(from, from + 999);
          if (error || !data || data.length === 0) break;
          allLots.push(...(data as RawLot[]));
          if (data.length < 1000) break;
          from += 1000;
        }
      }

      // Filter unsold in JS to avoid Supabase boolean quirk
      const unsoldLots = allLots.filter((l) => l.sold === false);
      ...
      // Extract unique departments and lot categories
      const depts = Array.from(new Set(unsoldLots.map((l) => l.department).filter((d): d is string => d !== null))).sort();
      const cats = Array.from(new Set(unsoldLots.map((l) => l.category).filter((c): c is string => c !== null))).sort();
  ...
  const getFilteredData = (): DescriberUnsold[] => {
    ...
    if (describerFilter !== "all" && dynamicFilters.length > 0) {
      const activeFilters = dynamicFilters.filter((f) => f.value !== "");
  ...
  const totalUnsoldAll = describerData.reduce((sum, d) => sum + d.lots.length, 0);

  const estimateRangeBreakdown = ESTIMATE_RANGES.map((range) => {
    const allLots = filteredData.flatMap((d) => d.lots);
    const count = allLots.filter((lot) => {
      const mid = (toNum(lot.estimateLow) + toNum(lot.estimateHigh)) / 2;
      if (mid < range.min) return false;
      if (range.max !== null && mid > range.max) return false;
      return true;
        ...
        {describerFilter !== "all" && (
          <div className="space-y-3 pt-3 border-t border-[#1e3a6b]">
            <div className="flex items-center justify-between">
              <p className="text-xs text-[#6687bc] font-medium">Additional filters — all combine with AND logic</p>
              <button
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="card-sm flex items-center gap-3 py-3">
              ...
              <div>
                <p className="text-xs text-[#6687bc]">Unsold (filtered)</p>
            ...
            <div className="card-sm flex items-center gap-3 py-3">
              <div>
                ...
                <p className="text-lg font-semibold text-[#f7f4ec]">{totalUnsoldAll}</p>
              </div>
            </div>
            <div className="card-sm flex items-center gap-3 py-3">
              <div>
                <p className="text-xs text-[#6687bc]">Describers shown</p>
